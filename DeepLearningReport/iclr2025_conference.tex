...
Task 16 (basic induction) asks the model to identify a pattern from a small set of examples (e.g.\ ``Wolves are afraid of cats. Cats are afraid of mice.'' $\rightarrow$ ``What are wolves afraid of?''). The paper achieves 0\% error on this task with MemN2N PE+LS using 10k training. Our single-run baseline gives 51.2\% --- essentially random guessing. The failure is not due to insufficient reasoning capacity; a 3-hop model is enough in principle. The more likely explanation is that the rule sentence is hard to locate without some guidance on where to look.
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.92\linewidth]{images/output.png}
\end{center}
\caption{Semi-supervised test error as a function of labeled ratio for Tasks 16, 17, and 19. The dotted line is the unsupervised MemN2N baseline. Task 16 drops sharply at 25\% labeled; Tasks 17 and 19 increase, suggesting that attention supervision hurts when the bottleneck is not attentional.}
\label{fig:ss}
\end{figure}

\paragraph{Task 16 --- supervision nearly solves it.}
At 25\% labeled, error drops from 51.2\% to 0.4\% --- matching the paper's 0.0\%. The improvement is sharp and appears between 10\% and 25\% labeled, suggesting there is a critical mass of annotated examples below which the model cannot reliably learn where the rule sentence is, but above which it converges cleanly. The attention visualization in Figure~\ref{fig:attn} shows that the supervised model (50\% labeled) concentrates its hop-1 attention on the induction rule sentence, while the unsupervised model spreads it almost uniformly across all sentences.
